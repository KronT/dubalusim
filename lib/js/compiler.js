...
out: while (args.length > 0) {
		...
		switch (v) {
			...
			default:
				if (filename) {
					sys.print("ERROR: input defined multiple times!\n");
					process.exit(1);
					return null;
				}
				filename = v;
...
if (filename) {
		fs.readFile(filename, "utf8", function(err, text){
				if (err) throw err;
				output(squeeze_it(check_it(preprocess_it(text))));
		});
} else {
...
function preprocess_it(code) {
		if (options.preprocess && filename) {
			var dirname = path.dirname(filename);
			code = code.replace(/@include\(("|')(.*)\1\);/g, function(a, b, filename) {
...
function check_it(code) {
	if (options.jshint) {
		if ( time_it("jshint", function() {
			return jshint( code, options.jshint_options );
		}) ) {
