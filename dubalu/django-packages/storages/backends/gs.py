...
class GSStorage(GSBotoStorage):
    ...
    def path(self, name):
        ...
        """
        _name = self._normalize_name(self._clean_name(name)).replace('/', '__')
        md5 = hashlib.md5(_name).hexdigest()
        pathname = os.path.join(self.paths, md5[:2])
        filename = os.path.join(pathname, _name)
        isdir = os.path.isdir(pathname)
        exists = isdir and os.path.exists(filename)
        if not isdir:
