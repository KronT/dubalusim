...
class TestFormLayout(CrispyTestCase):
    ...
    def test_second_layout_multifield_column_buttonholder_submit_div(self):
        ...
        c = Context({'form': form, 'form_helper': form_helper, 'value_var': "Save"})
        html = template.render(c)
        ...
        c = Context({'form': form, 'form_helper': form_helper, 'value_var': "Update"})
        html = template.render(c)
        self.assertTrue('value="Update"' in html)
    ...
    def test_formset_layout(self):
        ...
        if django_version < '1.7' and 0:  # Added by Kronuz: patched django has MIN_NUM backport !!
            self.assertEqual(html.count("hidden"), 4)
        else:
            self.assertEqual(html.count("hidden"), 5)
