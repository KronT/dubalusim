...
def flatten_list(tuples):
    return list(t for f in tuples for t in (f if isinstance(f, (list, tuple)) else (f,)))


def resolve_value(context, lookup, silent=True):
    try:
        fields = lookup.split('.')
        if not fields:
        ...
        def _resolve_value(value):
            if isinstance(value, dict):
                return dict((rk, rv) for rk, rv in ((k, _resolve_value(v)) for k, v in value.items() if k[0] != '_') if rv)

            if isinstance(value, (tuple, list)):
                return [rv for rv in (_resolve_value(v) for v in value) if rv]

            return value

        return _resolve_value(obj)

    except UnresolvedError:
        if not silent:
            if not FAIL_SILENTLY:
                raise Exception("Could not resolve value for '%s'." % lookup)
            else:
                logging.warning("Could not resolve value for '%s'." % lookup, exc_info=sys.exc_info())


def get_bound_field(context, obj, lookup):
    try:
        fields = lookup.split('.')
        if not fields:
            raise UnresolvedError
        ...
        for i, name in enumerate(fields):
            ...
            try:
                if name.isdigit():
                    obj = obj[int(name)]
                    continue
                bound_field = obj[name]
                if hasattr(bound_field.field, 'form'):
                    obj = bound_field.field.widget

            except (TypeError, KeyError, IndexError):
                if i != 0:
                    raise UnresolvedError
                # Fallback to find main object in the context:
                if context is None:
                    raise Exception("context should be given for fields with absolute names")
                obj = context[name]
        ...
        if not FAIL_SILENTLY:
            raise Exception("Could not resolve form field '%s'." % lookup)
        else:
            logging.warning("Could not resolve form field '%s'." % lookup, exc_info=sys.exc_info())


def render_field(field, form, form_style, context, template=None, labelclass=None, show_labels=None, layout_object=None, attrs=None, template_pack=TEMPLATE_PACK):
    """
