...
from webassets.cache import BaseCache, make_md5, safe_unpickle
from webassets.utils import pickle
try:
...
class FilesystemCache(BaseCache):
    ...
    def directory(self):
        try:
            return os.path.join(getattr(settings, 'CACHE_ROOT'), 'webassets-cache')
...
class DjangoConfigStorage(ConfigStorage):
    ...
    def __getitem__(self, key):
        if self.__contains__(key):
            ...
            if key == 'cache' and value is True:
                directory = getattr(settings, self._transform_key('directory'))
                value = FilesystemCache(os.path.join(directory, '.webassets-cache'))
            return value
        else:
    ...
    def __setitem__(self, key, value):
        if not self._set_deprecated(key, value):
            # Set default settings (not local settings, which are erased on every request):
            setattr(settings.default_settings, self._transform_key(key), value)
