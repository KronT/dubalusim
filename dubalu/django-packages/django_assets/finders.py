...
class AssetsFinder(staticfiles.finders.BaseStorageFinder):
    ...
    def list(self, ignore_patterns, include_patterns):
        ...
        if env.directory == getattr(settings, 'STATIC_ROOT'):
            for bundle in env:
                try:
                    ...
                    continue
                if matches_patterns(output, ignore_patterns):
                    continue
                if include_patterns and not matches_patterns(output, include_patterns):
                    continue
                if self.storage.exists(output):
            ...
            for output in super(AssetsFinder, self).list(ignore_patterns, include_patterns):
                yield output
