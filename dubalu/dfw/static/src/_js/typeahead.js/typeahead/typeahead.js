...
var Typeahead = (function() {
  ...
  function buildDomStructure(input, withHint) {
    var $input, $wrapper, $dropdown, $hint, attrs;

    $input = $(input);
    attrs = $input.prop('attributes');

    $wrapper = $(html.wrapper);
    // loop through <select> attributes and apply them on <div>
    $.each(attrs, function() {
      if (this.name !== 'title' && this.name.substring(0, 5) !== 'data-') {
        $wrapper.attr(this.name, this.value);
      }
    });
    $wrapper.css(css.wrapper).addClass('tt-wrapper');//.css(getBackgroundStyles($input));
    $dropdown = $(html.dropdown).css(css.dropdown);
    $hint = $input.clone(false, false).css(css.hint);
