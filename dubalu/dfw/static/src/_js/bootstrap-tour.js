  ...
  Tour = (function() {
    ...
    Tour.prototype.getStep = function(i) {
      if (this._options.steps[i] != null) {
        var prev = i - 1,
          prev_step = this._options.steps[prev],
          next = i + 1,
          next_step = this._options.steps[next];
        while (next_step !== undefined && next_step.skip) {
          next += 1;
          next_step = this._options.steps[next];
        }
        ...
        while (prev_step !== undefined && prev_step.skip) {
          prev -= 1;
          prev_step = this._options.steps[prev];
        }
        ...
        return $.extend({
          ...
          easein: this._options.easein,
          easeout: this._options.easeout,
    ...
    Tour.prototype._showNextStep = function() {
      ...
      promise = this._makePromise((step && step.onNext != null) ? step.onNext(this) : void 0);
    ...
    Tour.prototype._showPrevStep = function() {
      ...
      promise = this._makePromise((step && step.onPrev != null) ? step.onPrev(this) : void 0);
    ...
    Tour.prototype._showBackdrop = function(element) {
      ...
      return $("body").addClass('backdrop-shown').append(this.backdrop);
    };

    Tour.prototype._setFocus = function (step, set) {
      var element = $(step.element),
        focus = step.focus;
      if (typeof focus === 'string') {
        element = element.find(focus).first();
        if (!element.length) {
          element = $(focus).first();
        }
      }
      if (set) {
        ...
        element.removeClass('focus').blur();
      }
    };
    ...
    Tour.prototype._hideBackground = function() {
      ...
      $("body").removeClass('backdrop-shown');
    ...
    Tour.prototype._showOverlay = function(step) {
      ...
      $element = (typeof step.backdrop === 'string') ? $(step.backdrop) : $(step.element);
      ...
      var p = step.backdropPadding,
          s = (p && p.split) ? p.split(' ') : p,
          padding = {
            top: p && _firstof(p.top, s[0], p) || 0,
            left: p && _firstof(p.left, s[1], p) || 0,
            bottom: p && _firstof(p.bottom, s[2], s[0], p) || 0,
            right: p && _firstof(p.right, s[3], s[1], p) || 0
          };
      this.backdrop.overlayElementShown = true;
      $background = $("<div/>");
      offset = $element.offset();
      offset.top = offset.top - padding.top;
      offset.left = offset.left - padding.left;
      var bgsz = function() {
        $background
          .width($element.outerWidth() + padding.left + padding.right)
          .height($element.outerHeight() + padding.top + padding.bottom)
          .addClass("tour-step-background").offset(offset);
      };
      bgsz();
      $element.addClass("tour-step-backdrop").resize(bgsz);;
