...
class FileHunk(BaseHunk):
    ...
    def data(self):
        f = open(self.filename, 'rb' if self.binary else 'r', encoding=None if self.binary else 'utf-8')
        try:
            return f.read()
        finally:
            f.close()
...
class UrlHunk(BaseHunk):
    ...
    def __init__(self, url, env=None, binary=False):
        self.url = url
        self.env = env
        self.binary = binary
...
class MemoryHunk(BaseHunk):
    ...
    def __init__(self, data, files=None, binary=False):
        self._data = data
        self.files = files or []
        self.binary = binary
    ...
    def save(self, filename):
        f = open(filename, 'wb' if self.binary else 'w', encoding=None if self.binary else 'utf-8')
        try:
            f.write(self.data())
        finally:
            f.close()
...
def merge(hunks, separator=None, binary=False):
    ...
    if not separator:
        separator = '\n'
    return MemoryHunk(separator.join([h.data() for h in hunks]), binary=binary)
