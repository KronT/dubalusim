...
class Bundle(object):
    ...
    def resolve_output(self, ctx=None, version=None):
        ...
        if self.merge:
            ...
            output = ctx.resolver.resolve_source(ctx, self.output)
    ...
    def _merge_and_apply(self, ctx, output, force, parent_debug=None,
                         ...
                         disable_cache=None, binary=False):
        ...
        for item, cnt in resolved_contents:
            if isinstance(cnt, Bundle):
                ...
                hunk = cnt._merge_and_apply(
                    ...
                    filters_to_pass_down, disable_cache=disable_cache, binary=binary)
                if hunk is not None:
                    hunks.append((hunk, {}))
                ...
                try:
                    hunk = filtertool.apply_func(
                        ...
                        cache_key=[FileHunk(cnt, binary=binary)] if not is_url(cnt) else [])
                    ...
                    if is_url(cnt):
                        ...
                        hunk = FileHunk(cnt, binary=binary)
        ...
        try:
            try:
                ...
                final = merge([h for h, _ in hunks], binary=binary)
        except IOError as e:
    ...
    def _build(self, ctx, extra_filters=None, force=None, output=None,
        ...
        if not update_needed:
            ...
            return FileHunk(self.resolve_output(ctx, self.output), binary=binary)

        hunk = self._merge_and_apply(
            ctx, [self.output, self.resolve_output(ctx, version='?')],
            force, disable_cache=disable_cache, extra_filters=extra_filters, binary=binary)
        if hunk is None:
    ...
    def build(self, force=None, output=None, disable_cache=None, binary=False):
        ...
        for bundle, extra_filters, new_ctx in self.iterbuild(ctx):
            hunks.append(bundle._build(
                ...
                disable_cache=disable_cache, binary=binary))
        return hunks
        ...
        if supposed_to_merge and (self.filters or self.output):
            ...
            for org, cnt in self.resolve_contents(ctx):
                if isinstance(cnt, Bundle):
                    ...
                    try:
                        url = ctx.resolver.resolve_source_to_url(ctx, cnt, url)
                    except ValueError:
                        ...
                        url = ctx.resolver.resolve_source_to_url(ctx, external, url)

                    if version and ctx.url_expire or (
                            ctx.url_expire is None and not has_placeholder(org)):
                        url = "%s?%s" % (url, version)
    ...
    def urls(self, *args, **kwargs):
        ...
        behind these urls.
        """
        env = kwargs.get('env') or self.env
        ctx = wrap(env, self)
        urls = []
        for bundle, extra_filters, new_ctx in self.iterbuild(ctx):
