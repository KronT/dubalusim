...
class PyScss(Filter):
    ...
    *Supported configuration options:*
    ...
    PYSCSS_STYLE (style)
        Setup pyscss active style.

        Values can be 'legacy', 'compressed', 'compact', 'expanded' or 'nested'
    ...
    def setup(self):
        ...
        try:
            scss.config.STATIC_ROOT = self.static_root or static_finder or self.ctx.directory
        ...
        scss.config.ASSETS_ROOT = self.assets_root or self.static_root or self.ctx.directory
        scss.config.ASSETS_URL = self.assets_url or self.static_url or self.ctx.url
